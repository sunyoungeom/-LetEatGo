...
public class ChatServer extends HttpServlet {
	private Map<Integer, List<Session>> clients = new HashMap<>();
	private User user;
...
//		String postId = session.getRequestParameterMap().get("postId").get(0);
				...
				for (List<Session> sessionList : clients.values()) {
					for (Session c : sessionList) {
						...
						if (nickname.equals(receiverId)) {
							// 귓속말을 받을 대상에게 메시지 전송
							c.getBasicRemote().sendText("[귓속말] " + session.getUserProperties().get(user.getNickname())
									+ ": " + whisperContent);
							// 메시지를 보낸 사람에게도 본인이 보낸 메시지로 전송
							session.getBasicRemote()
									.sendText("[귓속말] " + session.getUserProperties().get(user.getNickname()) + " -> "
											+ receiverId + ": " + whisperContent);
