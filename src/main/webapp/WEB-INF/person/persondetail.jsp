...
<script>
	...
	document.addEventListener("DOMContentLoaded", function() {
        ...
        if (progressingRadio.checked) {
            const fetchURI = `http://localhost:8080/person/activepostlist?page=${currentPage}&pagePer=${itemsPerPage}&userid=${userid}`;
	...
	completedRadio.addEventListener("change", () => {
	    ...
	    if (completedRadio.checked) {
	   		const fetchURI = `http://localhost:8080/person/completedpostlist?page=${currentPage}&pagePer=${itemsPerPage}&userid=${userid}`;
	    	loadPosts(currentPage, fetchURI);
	    } 
	});
	loadPosts(currentPage, fetchURI);
